...
export function DashboardLayout({ children, title, noPadding }: DashboardLayoutProps) {
  ...
  return (
    <SidebarProvider defaultOpen={false}>
      <div className={noPadding ? "h-screen overflow-hidden flex w-full bg-background" : "min-h-screen flex w-full bg-background"}>
        <AppSidebar />
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main className={noPadding ? "flex-1 min-h-0 overflow-hidden" : "flex-1 flex flex-col p-4 sm:p-6"}>
            {noPadding ? (
              children
            ) : (
              <>
